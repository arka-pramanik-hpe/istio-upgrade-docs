...
# TODO: Upgrade to 1.16.7 using Helm
...
```sh
helm upgrade istio-base manifests/charts/base -n istio-system --skip-crds --version 1.16.7
```
4. Upgrade the Istio discovery chart:
```sh
helm upgrade istiod istio/istiod -n istio-system --version 1.16.7
```
5. Upgrade and gateway charts installed in your cluster:
```sh
helm upgrade istio-ingress istio/gateway -n istio-ingress --version 1.16.7
```
...
Events:
  ...
  Normal   Pulled            38m                   kubelet            Container image "docker.io/istio/proxyv2:1.16.7" already present on machine
...
We can see istio proxy version has been upgraded to 1.16.7
